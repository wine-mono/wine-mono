...
$$(BUILDDIR)/wpfgfx-$(1)/.built: $$(WPF_SRCS) $$(MINGW_DEPS)
	mkdir -p $$(@D)
	+$$(MINGW_ENV) CFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" CXXFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" LDFLAGS="$$(PDB_LDFLAGS_$(1))" $(MAKE) -C $$(@D) -f $$(SRCDIR_ABS)/wpf/wpfgfx/Makefile ARCH=$(1) SRCDIR="$$(SRCDIR_ABS)/wpf/wpfgfx" "MINGW=$$(MINGW_$(1))"
	touch "$$@"
...
$$(BUILDDIR)/PresentationNative-$(1)/.built: $$(WPF_SRCS) $$(MINGW_DEPS)
	mkdir -p $$(@D)
	+$$(MINGW_ENV) CFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" CXXFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" LDFLAGS="$$(PDB_LDFLAGS_$(1))" $(MAKE) -C $$(@D) -f $$(SRCDIR_ABS)/wpf/PresentationNative/Makefile ARCH=$(1) SRCDIR="$$(SRCDIR_ABS)/wpf/PresentationNative" "MINGW=$$(MINGW_$(1))"
	touch "$$@"
ifeq (1,$(ENABLE_DOTNET_CORE_WPF))
...
$$(BUILDDIR)/wmwpfdwhelper-$(1)/.built: $$(WPF_SRCS) $$(MINGW_DEPS)
	mkdir -p $$(@D)
	+$$(MINGW_ENV) CFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" CXXFLAGS="$$(PDB_CFLAGS_$(1)) $$(OPT_CFLAGS)" LDFLAGS="$$(PDB_LDFLAGS_$(1))" $(MAKE) -C $$(@D) -f $$(SRCDIR_ABS)/wpf/wmwpfdwhelper/Makefile ARCH=$(1) SRCDIR="$$(SRCDIR_ABS)/wpf/wmwpfdwhelper" "MINGW=$$(MINGW_$(1))"
	touch "$$@"
ifeq (1,$(ENABLE_DOTNET_CORE_WPF))
...
IMAGEDIR_BUILD_TARGETS += $(SRCDIR)/wpf/src/Microsoft.DotNet.Wpf/src/UIAutomation/UIAutomationClient/.built

UIAutomationClient.dll: $(SRCDIR)/wpf/src/Microsoft.DotNet.Wpf/src/UIAutomation/UIAutomationClient/.built
	$(MONO_ENV) gacutil -i $(SRCDIR)/wpf/src/Microsoft.DotNet.Wpf/src/UIAutomation/UIAutomationClient/UIAutomationClient.dll -root $(IMAGEDIR)/lib
.PHONY: UIAutomationClient.dll
imagedir-targets: UIAutomationClient.dll
