...
$$(BUILDDIR)/monodx-$(1)/.built: $$(MONODX_SRCS) $$(MINGW_DEPS)
	mkdir -p $$(@D)
	+$$(MINGW_ENV) CFLAGS="$$(PDB_CFLAGS_$(1)) -O2" CXXFLAGS="$$(PDB_CFLAGS_$(1)) -O2" LDFLAGS="$$(PDB_LDFLAGS_$(1))" $(MAKE) -C $$(@D) -f $$(SRCDIR_ABS)/monoDX/monodx/Makefile ARCH=$(1) SRCDIR="$$(SRCDIR_ABS)/monoDX/monodx" "MINGW=$$(MINGW_$(1))"
	touch "$$@"
